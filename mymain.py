...
if __name__ == '__main__':
    ...
    store_dir = os.path.join("stored_data_res", "MeanTeacher" + add_dir_model_name)
    saved_model_dir = os.path.join(store_dir, "model_pool")
    ...
    # 以下四个initialize都是进行mel预处理并保存成npy文件，并返回新的df（有可能剔除一些无效的文件）
    ...
    model = CRNN(**crnn_kwargs)
    model_ema = CRNN(**crnn_kwargs)
    ...
    state = {
        ...
        'model_ema': {"name": model_ema.__class__.__name__,
                      'args': '',
                      "kwargs": crnn_kwargs,
    ...
    for epoch in range(cfg.n_epoch):
        ...
        train(training_data, model, optimizer, epoch, ema_model=model_ema, weak_mask=weak_mask, strong_mask=strong_mask)

        model = model.eval()

        LOG.info("\n ### Valid synthetic metric ### \n")
        predictions = get_predictions(model, valid_synth_data, many_hot_encoder.decode_strong,
                                      save_predictions=None)
        valid_events_metric = compute_strong_metrics(predictions, valid_synth_df, pooling_time_ratio)

        LOG.info("\n ### Valid weak metric ### \n")
        weak_metric = get_f_measure_by_class(model, len(classes),
